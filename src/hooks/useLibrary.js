...
import { parseFilename, checkDuration, getSuggestedPath, MIN_DURATION_SECONDS } from '../utils/parser';
import { fetchTMDB } from '../utils/tmdb';

const STORAGE_KEY = 'filmsort_library_v2';
...
export function useLibrary() {
  ...
  const processFiles = useCallback(async (files) => {
    ...
    for (let i = 0; i < initial.length; i++) {
      ...
      setQueue(q => q.map((r, idx) => idx === i ? { ...r, status: 'checking' } : r));
      const duration = await getVideoDuration(qItem.file);
      const durationCheck = checkDuration(duration);

      if (!durationCheck.ok) {
        setQueue(q => q.map((r, idx) => idx === i ? { ...r, status: 'rejected', rejectReason: durationCheck.reason } : r));
        setProcessingCount(c => c - 1);
        continue;
      }

      // Step 2: Parse filename
      setQueue(q => q.map((r, idx) => idx === i ? { ...r, status: 'analyzing' } : r));
      ...
      try {
        ...
        results.push(entry);
        setQueue(q => q.map((r, idx) => idx === i ? { ...r, status: 'done', parsed } : r));
      } catch (err) {
        console.error('Parse error:', err);
        setQueue(q => q.map((r, idx) => idx === i ? { ...r, status: 'error' } : r));
      }
  ...
  const recentlyWatched = library.filter(i => i.lastWatched).sort((a,b) => new Date(b.lastWatched) - new Date(a.lastWatched)).slice(0, 20);
  ...
  const seriesMap = tvShows.reduce((acc, ep) => {
    ...
    if (!acc[key]) acc[key] = { title: ep.title, season: ep.seasonNumber, type: ep.type, episodes: [], poster: ep.poster, backdrop: ep.backdrop, genre: ep.genre, overview: ep.overview };
    acc[key].episodes.push(ep);
    return acc;
  }, {});
  const series = Object.values(seriesMap).map(s => ({
    ...s,
    episodes: s.episodes.sort((a,b) => (a.episodeNumber||0) - (b.episodeNumber||0)),
  ...
  return {
    ...
    processFiles, updateItem, removeItem, clearLibrary,
    movies, tvShows, series, watching, recentlyWatched, recentlyAdded, watched,
  };
}
