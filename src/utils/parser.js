...
function stripResolutions(s) { return s.replace(RESOLUTION_PATTERN, ' '); }
...
const SMALL_WORDS = new Set(['a','an','the','and','but','or','for','nor','on','at','to','by','of','in','is','vs','via']);
const ROMAN = { 'i':'I','ii':'II','iii':'III','iv':'IV','v':'V','vi':'VI','vii':'VII','viii':'VIII','ix':'IX','x':'X' };
...
function formatPath(item) {
  if (item.type === 'tv') {
    ...
    const eEnd = item.episodeEnd ? `-E${String(item.episodeEnd).padStart(2,'0')}` : '';
...
export function parseFilename(originalFilename) {
  const hasMovieKW = /\b(?:the\.?movie|motion\.?picture|telesync|hdcam|screener|dvdscr|cam\.?rip)\b/i.test(originalFilename);
  const isAnimeSource = /\b(?:crunchyroll|funimation|hidive|vrv|anidb)\b/i.test(originalFilename);

  const cleaned = normalizeSpacing(stripAll(originalFilename));

  // ── 1. Multi-episode: S01E01E02, S01E01-E02, S01E01-03 ────────────────
  const multiEpPattern = /^(.+?)\s+S(\d{1,2})\s*E(\d{1,3})[-–]E?(\d{1,3})\s*(?:-\s*)?(.*)$/i;
  const multiM = cleaned.match(multiEpPattern);
  if (multiM) {
    const show = titleCase(multiM[1].trim());
    const sn = parseInt(multiM[2]), en = parseInt(multiM[3]), enEnd = parseInt(multiM[4]);
    let epTitle = multiM[5] ? titleCase(multiM[5].trim()) : null;
    if (epTitle && epTitle.length < 2) epTitle = null;
    return {
      title: show, type: 'tv', year: extractYear(multiM[1]),
      seasonNumber: sn, episodeNumber: en, episodeEnd: enEnd,
      episodeTitle: epTitle, confidence: 97,
  ...
  if (tvM) {
    ...
    if (epTitle && (epTitle.length < 2 || /^[se]$/i.test(epTitle))) epTitle = null;
    const sn = parseInt(tvM[2]), en = parseInt(tvM[3]);
    return {
      title: show, type: isAnimeSource ? 'anime' : 'tv',
      year: extractYear(tvM[1]),
      seasonNumber: sn, episodeNumber: en, episodeEnd: null,
      episodeTitle: epTitle, confidence: 97,
  ...
  // ── 3. 1x05 format ─────────────────────────────────────────────────────
  const altTvM = cleaned.match(/^(.+?)\s+(\d{1,2})x(\d{1,3})\s*(?:-\s*)?(.*)$/i);
  if (altTvM) {
    const show = titleCase(altTvM[1].trim());
    const sn = parseInt(altTvM[2]), en = parseInt(altTvM[3]);
    let epTitle = altTvM[4] ? titleCase(altTvM[4].trim()) : null;
    if (epTitle && epTitle.length < 2) epTitle = null;
    return {
      title: show, type: 'tv', year: extractYear(altTvM[1]),
      seasonNumber: sn, episodeNumber: en, episodeEnd: null,
      episodeTitle: epTitle, confidence: 93,
      cleanedFilename: `${show} - S${String(sn).padStart(2,'0')}E${String(en).padStart(2,'0')}.mp4`,
    };
  }

  // ── 4. Season N Episode N ──────────────────────────────────────────────
  const longTvM = cleaned.match(/^(.+?)\s+Season\s*(\d{1,2})\s+Episode\s*(\d{1,3})\s*(?:-\s*)?(.*)$/i);
  if (longTvM) {
    const show = titleCase(longTvM[1].trim());
    const sn = parseInt(longTvM[2]), en = parseInt(longTvM[3]);
    return {
      title: show, type: 'tv', year: extractYear(longTvM[1]),
      seasonNumber: sn, episodeNumber: en, episodeEnd: null,
  ...
  const ovaM = cleaned.match(/^(.+?)\s+(?:OVA|ONA|OAD|Special|Movie|Film)\s*(\d{0,3})\s*(?:-\s*)?(.*)$/i);
  if (ovaM && !hasMovieKW) {
    const show = titleCase(ovaM[1].trim());
    const label = cleaned.match(/\b(OVA|ONA|OAD|Special|Movie)\b/i)?.[1]?.toUpperCase() || 'SPECIAL';
    return {
      title: show, type: 'anime', year: null,
      seasonNumber: 0, episodeNumber: ovaM[2] ? parseInt(ovaM[2]) : 1,
  ...
  if (animeHighEp && !hasMovieKW) {
    ...
    if (!isRes && epNum <= 2000) {
      ...
      return {
        title: show, type: 'anime', year: null,
        seasonNumber: season || 1, episodeNumber: epNum, episodeEnd: null,
        episodeTitle: null, confidence: 87,
  ...
  if (animeM && !hasMovieKW) {
    ...
    if (!isRes && epNum >= 1 && epNum < 500) {
      ...
      return {
        title: show, type: 'anime', year: null,
        seasonNumber: sn, episodeNumber: epNum, episodeEnd: null,
  ...
  if (standM && !hasMovieKW) {
    ...
    if (epNum >= 1 && epNum <= 99) {
      ...
      return {
        title: show, type: 'anime', year: null,
        seasonNumber: season || 1, episodeNumber: epNum, episodeEnd: null,
        episodeTitle: null, confidence: 78,
        cleanedFilename: `${show} - ${String(epNum).padStart(2,'0')}.mp4`,
      };
    }
  }
  ...
  // ── 9. E## only ────────────────────────────────────────────────────────
  const epOnlyM = cleaned.match(/^(.+?)\s*[-–]?\s*E(\d{1,3})(?:\s*[-–]\s*(.+))?$/i);
  if (epOnlyM && !hasMovieKW) {
    ...
    if (epNum > 0 && epNum < 500) {
      ...
      if (epTitle && epTitle.length < 2) epTitle = null;
      return {
        title: show, type: 'tv', year: null,
        seasonNumber: sn, episodeNumber: epNum, episodeEnd: null,
        episodeTitle: epTitle, confidence: 83,
  ...
  const partM = cleaned.match(/^(.+?)\s+(?:Part|Pt)\.?\s*(\d+|[IVX]+)\s*(?:[-–:]\s*(.+))?$/i);
  if (partM && !partM[1].match(/S\d{1,2}$/i)) {
    ...
    return {
      title: movieTitle, type: 'movie',
      year: extractYear(partM[1]),
      seasonNumber: null, episodeNumber: null, episodeEnd: null,
      episodeTitle: null, confidence: 82,
  ...
  // ── 11. Movie with year ─────────────────────────────────────────────────
  const yearM = cleaned.match(/^(.+?)\s+[\(\[]?(19\d{2}|20[012]\d)[\)\]]?\s*(.*)$/);
  if (yearM || hasMovieKW) {
    let mt = cleaned, year = null;
    if (yearM) { mt = yearM[1].trim(); year = parseInt(yearM[2]); }
    mt = mt.replace(/\s*[-–]?\s*the\s*movie\s*/gi, ' ').trim();
    mt = titleCase(mt);
    return {
      title: mt, type: 'movie', year,
      seasonNumber: null, episodeNumber: null, episodeEnd: null,
      episodeTitle: null, confidence: hasMovieKW ? 95 : 86,
  ...
  if (ft.length > 2 && ft.length < 120) {
    return {
      title: ft, type: 'movie', year: null,
      seasonNumber: null, episodeNumber: null, episodeEnd: null,
      episodeTitle: null, confidence: 38,
      cleanedFilename: `${ft}.mp4`,
    };
  }

  return {
    title: originalFilename, type: 'unknown', year: null,
    seasonNumber: null, episodeNumber: null, episodeEnd: null,
    episodeTitle: null, confidence: 0,
    cleanedFilename: originalFilename,
  };
}

// ── Duration filter ────────────────────────────────────────────────────
export const MIN_DURATION_SECONDS = 20 * 60; // 20 minutes

export function checkDuration(durationSeconds) {
  if (durationSeconds === null || durationSeconds === undefined) return { ok: true, reason: null };
  if (durationSeconds < MIN_DURATION_SECONDS) {
    const mins = Math.round(durationSeconds / 60);
    return { ok: false, reason: `Too short (${mins} min) — FilmSort only indexes videos ≥20 min` };
  }
