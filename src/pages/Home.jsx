...
import { Carousel, PosterCard, WideCard, SeriesCard } from '../components/Cards';
import Onboarding from '../components/Onboarding';
import DetailModal from '../components/DetailModal';
import EpisodeModal from '../components/EpisodeModal';
import { IconFilm, IconTV, IconClock, IconSparkle, IconPause } from '../components/Icons';

export default function Home({ library, movies, series, watching, recentlyWatched, recentlyAdded, updateItem, processFiles }) {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingBottom: 'clamp(24px,4vw,60px)' }}>
      ...
      <div className="page-pad" style={{
        marginTop: -10, marginBottom: 28, position: 'relative', zIndex: 5,
        ...
        {/* Stats strip */}
        <div style={{ display: 'flex', gap: 'clamp(12px,3vw,22px)', alignItems: 'baseline', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={l} style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(16px,3.5vw,20px)', color: 'var(--text)' }}>{n}</span>
      ...
      {watching.length > 0 && (
        <Carousel title="Continue Watching" icon={IconPause}>
          {watching.map(item => <WideCard key={item.id} item={item} onClick={setSelectedItem} />)}
        </Carousel>
      )}
      ...
      {recentlyWatched.length > 0 && (
        <Carousel title="Recently Watched" icon={IconClock}>
          {recentlyWatched.map(item => <WideCard key={item.id} item={item} onClick={setSelectedItem} />)}
        </Carousel>
      )}
      ...
      {showMovies && movies.length > 0 && (
        <Carousel title="Movies" icon={IconFilm}>
          {movies.map(item => <PosterCard key={item.id} item={item} onClick={setSelectedItem} />)}
        </Carousel>
      )}
      ...
      {showSeries && series.length > 0 && (
        <Carousel title="Series & Anime" icon={IconTV}>
          {series.map(s => <SeriesCard key={s.id} series={s} onClick={setSelectedSeries} />)}
        </Carousel>
      )}
      ...
      {recentlyAdded.length > 0 && (
        <Carousel title="Recently Added" icon={IconSparkle}>
          {recentlyAdded.map(item => <PosterCard key={item.id} item={item} onClick={setSelectedItem} />)}
        </Carousel>
      )}
